...
class PosSession(models.Model):
    ...
    pos_display_product_wise_detail = fields.Boolean(related='company_id.res_display_product_wise_detail', readonly=False)
    ...
    def get_real_times(self,tz):
        tz = pytz.timezone(str(tz))
        current_time = datetime.now(tz)
        return current_time.strftime(DEFAULT_SERVER_DATETIME_FORMAT)
    
    def get_user_time_zone(self,time,tz):
        server_time = datetime.strptime(time, "%Y-%m-%d %H:%M:%S")
        server_time_utc = pytz.utc.localize(server_time)
        user_time_zone = pytz.timezone(tz)
        local_time = server_time_utc.astimezone(user_time_zone)
        local_time_str = local_time.strftime("%Y-%m-%d %H:%M:%S")

        return local_time_str

    def get_current_datetime(self):
        tz = pytz.timezone(str(self.user_id.tz))
        current_time = datetime.now(tz)
        return current_time.strftime(DEFAULT_SERVER_DATETIME_FORMAT)
        
    def get_opened_date(self):
        server_time = datetime.strptime(str(self.start_at), "%Y-%m-%d %H:%M:%S")
        server_time_utc = pytz.utc.localize(server_time)
        user_time_zone = pytz.timezone(str(self.user_id.tz))
        local_time = server_time_utc.astimezone(user_time_zone)
        local_time_str = local_time.strftime("%Y-%m-%d %H:%M:%S")
        return local_time_str
    ...
    def get_closed_date(self):
        if self.stop_at:
            server_time = datetime.strptime(str(self.stop_at), "%Y-%m-%d %H:%M:%S")
            server_time_utc = pytz.utc.localize(server_time)
            user_time_zone = pytz.timezone(str(self.user_id.tz))
            local_time = server_time_utc.astimezone(user_time_zone)
            local_time_str = local_time.strftime("%Y-%m-%d %H:%M:%S")
    ...
    def send_session_receipt_email(self, email_address, session_id):
        if email_address:
            ...
            report_ref = self.env.ref('bi_pos_closed_session_reports.action_report_session_z')
            report = report_ref._render_qweb_pdf(pos_session.id)
