...
class ResConfigSettings(models.TransientModel):
    """
        This class inherits from the `res.config.settings` model in Odoo and adds two boolean
        fields to the configuration settings page: `create_invoice_delivery_validate`
            ...
            if any(rec.product_id.invoice_policy == 'delivery' for rec in
                ...
                if invoice_created:
                    ...
                    if auto_send_invoice and invoice_created.partner_id.email:
                        ...
                        template.send_mail(
                            invoice_created.id, force_send=True,
                            email_values={
