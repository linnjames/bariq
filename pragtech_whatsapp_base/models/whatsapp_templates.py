...
class WhatsappTemplates(models.Model):
    ...
    provider = fields.Selection([('whatsapp_chat_api', 'Whatsapp Chat Api'), ('gupshup', 'Gupshup'), ('meta', 'Meta')], string="Provider")
    ...
    meta_sample_message = fields.Text(string="Meta Sample Message")
    meta_template_labels = fields.Char(string='Template Labels')
    ...
    def action_export_template_to_meta(self):
        # Export template on meta api

        business_account_id = self.whatsapp_instance_id.meta_whatsapp_business_account_id
        access_token = self.whatsapp_instance_id.whatsapp_meta_api_token
        ...
        if self.meta_sample_message:
            separate_content = self.meta_sample_message.split(",")
            if self.header == 'media_document':
                template_data = {
                    "name": self.name,
                    "components": [
                        {
                ...
                template_data_list = list(template_data)
                add_template_response = requests.post(url, data=json.dumps(template_data_list), headers=req_headers)
                if add_template_response.status_code == 200:
                    _logger.info("\nAdd templates successfully in Meta api")
            return True
    ...
    def action_view_meta_template(self):
        context = dict(self.env.context)
        context['default_template_labels'] = self.meta_template_labels
        context['default_template_category'] = self.category
        context['default_template_type'] = self.header
        context['default_languages'] = self.languages.name
        context['default_element_name'] = self.name
        ...
        context['default_interactive_actions'] = self.interactive_actions
        context['default_quick_reply1'] = self.quick_reply1
        context['default_sample_message'] = self.meta_sample_message
        if self.sample_url:
            context['default_add_sample_media_message'] = 'Download the document from the Sample URL and upload it to meta'
            context['default_sample_url'] = self.sample_url

        return {
            'type': 'ir.actions.act_window',
            'name': 'View Meta Template',
            'res_model': 'whatsapp.meta.templates',
            'view_mode': 'form',
            'view_id': self.env['ir.model.data']._xmlid_to_res_id('pragtech_whatsapp_base.whatsapp_meta_templates_wizard_form'),
            'target': 'new',
            'nodestroy': True,
            'context': context,
        }